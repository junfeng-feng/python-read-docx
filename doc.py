...
writeFileFlag = True
testFlag = False
selectedList = ["√", "0052",  "F0A2", "003C3A24", "007752F5"]
...
def replaceText(text):
    ...
    "").replace("请记录", "").replace("座机", "").replace("；", "").replace("✅", "")
...
#         baseSatisfy = -1
    ...
    fileResult.write(getTextByPosition(data,4,1,0)+",")#移动电话
    fileResult.write(",")#座机
    ...
    fileResult.write( data[10][0][27].text+",")  #  报灾过程
    fileResult.write(data[10][0][35].text+data[10][0][36].text+",")  # 救灾过程
    ...
    fileResult.write(data[12][0][30].text+data[12][0][31].text+data[12][0][32].text+",")  # 您最希望得到哪方面的培训
    ...
    fileResult.write( data[14][0][37].text if (isSelected(    data[14][0][37])) else "");fileResult.write(",")
