...
#     baseSatisfy = 0
#     if getParagrahByPostion(data,8,0,base8Pno+70).text.find("如不满意，您觉得都存在哪些问题") !=1:
#         baseSatisfy = -1
    fileResult.write(",")  # 序号
    ...
    fileResult.write(simpleSelect(getParagrahByPostion(data,8,0,2)) + ",")  # 是否为原民政灾害信息员
    fileResult.write((
                 "不清楚" if(not isSelected(getParagrahByPostion(data,8,0,7))) else "否"
          ) if(not isSelected(getParagrahByPostion(data,8,0,6))) else "是");fileResult.write(",")  # 是否为地震速报员
    fileResult.write(simpleSelect(getParagrahByPostion(data,8,0,11)) + ",")  # 是否为安全员
    fileResult.write(
        (
             "专职安全员"  if(isSelected(getParagrahByPostion(data,8,0,14)))  else "安全巡查员" 
          ) if (isSelected(getParagrahByPostion(data,8,0,11))) else "");fileResult.write(",")  # 如是，继续提问
    fileResult.write("否"  if(not isSelected(getParagrahByPostion(data,8,0,18))) else  "是");fileResult.write(",")  # 是否为专职灾害信息员
    fileResult.write(
             replaceText(getParagrahByPostion(data,8,0,19).text)  if(not isSelected(getParagrahByPostion(data,8,0,14))) else  "");fileResult.write(",")  
    fileResult.write(
               "" if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 24))) else "3小时以上"
              if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 23))) else "2-3小时"
              if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 22))) else  "1小时以下") ;fileResult.write(",")  # 如为兼职，每天用于灾害信息员的工作时长
    fileResult.write(replaceText(getParagrahByPostion(data,8,0,base8Pno + 27).text) + ","),  # 如为兼职，您的本职工作是
    fileResult.write(
        "" if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 33))) else "5-10年"
        if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 32))) else "3-5年"
        if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 31))) else  "1-3年"
        if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 30))) else  "少于1年" 
        if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 34))) else  "10年以上") ;fileResult.write(",")  # 担任灾害信息员的工作年限
    ...
    fileResult.write(
            "" if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 46))) else "否"
            if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 47))) else "单位安排"
            if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 48))) else  "个人安排") ;fileResult.write(",")  # 每年是否安排体检
    fileResult.write(
             "" if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 51))) else "非常健康"
             if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 52))) else "健康"
             if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 53))) else  "亚健康"
             if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 54))) else  "健康状况较差") ;fileResult.write(",")  # 您认为，目前自己的健康状况如何
    ...
    fileResult.write(
         "" if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 70))) else "不满意"
        if(not isSelected(getParagrahByPostion(data,8,0,base8Pno + 69))) else "满意" 
    ...
    fileResult.write(simpleSelect(getParagrahByPostion(data,10,0,23)) + ",")  # 是否上报过自然灾害灾情信息
    ...
    fileResult.write(simpleSelect(getParagrahByPostion(data,12,0,1)) + ",") ,  # 是否接受过灾害信息员的相关培训
    ...
    fileResult.write(replaceText(getParagrahByPostion(data,14,0,43).text) + ",")  # 您最急切的需要解决的问题是什么？
